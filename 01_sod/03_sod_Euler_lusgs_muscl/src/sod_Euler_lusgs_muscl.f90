...
module globals
  ...
  subroutine inner_ite(lQc)
    ...
    do ttt = 1, 10
       ...
       do while (con == 0)
          ...
          do i = 0, nx-1
             L(i,0) = dt*0.5d0*( Amatrix_plus(i,0,0)*delta_Q(i,0) &
                                +Amatrix_plus(i,0,1)*delta_Q(i,1) &
                                +Amatrix_plus(i,0,2)*delta_Q(i,2) )
             L(i,1) = dt*0.5d0*( Amatrix_plus(i,1,0)*delta_Q(i,0) &
                                +Amatrix_plus(i,1,1)*delta_Q(i,1) & 
                                +Amatrix_plus(i,1,2)*delta_Q(i,2) )
             L(i,2) = dt*0.5d0*( Amatrix_plus(i,2,0)*delta_Q(i,0) &
                                +Amatrix_plus(i,2,1)*delta_Q(i,1) &
                                +Amatrix_plus(i,2,2)*delta_Q(i,2) )
             D(i) = 2.0d0*dx + dt*beta_sigma(i)
             U(i,0) = dt*0.5d0*( Amatrix_minus(i,0,0)*delta_Q(i,0) &
                                +Amatrix_minus(i,0,1)*delta_Q(i,1) &
                                +Amatrix_minus(i,0,2)*delta_Q(i,2) )
             U(i,1) = dt*0.5d0*( Amatrix_minus(i,1,0)*delta_Q(i,0) &
                                +Amatrix_minus(i,1,1)*delta_Q(i,1) &
                                +Amatrix_minus(i,1,2)*delta_Q(i,2) )
             U(i,2) = dt*0.5d0*( Amatrix_minus(i,2,0)*delta_Q(i,0) &
                                +Amatrix_minus(i,2,1)*delta_Q(i,1) &
                                +Amatrix_minus(i,2,2)*delta_Q(i,2) )
          end do
  ...
  subroutine muscl(lQc)
    ...
    real(8), intent(in) :: lQc(0:nx-1,0:2)
    integer :: i, j
    real(8) :: dplus_j, dminus_j, dplus_jp, dminus_jp, minmod
  ...
  real(8)  function minmod(x, y, b)
    ...
    if ( x == 0.0 ) then
      ...
      minmod = sign(1.0_8, x)*max(0.0, min(abs(x), sign(1.0_8, x)*y*b))
    end if
  end function minmod
  ...
  subroutine Qctoqf(Qc_in, qf_out)
    ...
    do i = 0, nx-1
      ...
      qf_out(i,2) = (gamma - 1.0d0) * (Qc_in(i,2) - 0.5d0 * Qc_in(i,0)*vel*vel)
    end do
